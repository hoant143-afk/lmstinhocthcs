...
import { TaskTypeBadge, LearningModeBadge } from '../../components/common/Badge';
    ...
    <div className="space-y-6 pb-16">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <Link
            to={lesson.classId ? `/app/class/${lesson.classId}` : '/app'}
      ...
      {/* Online Meeting / Offline Location Banner */}
      {lesson.learningMode === 'online' && lesson.onlineMeetingUrl && (
        <div className="p-4 rounded-2xl bg-blue-50 border border-blue-200 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-sm font-bold text-blue-900">Phòng Học Trực Tuyến (Online Meeting)</div>
              <div className="text-xs text-blue-700 truncate max-w-lg">{lesson.onlineMeetingUrl}</div>
            </div>
          </div>
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center justify-center gap-1.5 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-bold text-xs rounded-xl shadow-xs transition"
          >
      ...
      {lesson.learningMode === 'offline' && lesson.location && (
        <div className="p-4 rounded-2xl bg-emerald-50 border border-emerald-200 flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-emerald-600 text-white flex items-center justify-center shrink-0">
            <MapPin className="w-5 h-5" />
          </div>
          <div>
            <div className="text-sm font-bold text-emerald-900">Địa Điểm Học Trực Tiếp</div>
            <div className="text-xs text-emerald-800 font-medium">{lesson.location}</div>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {activeTask.durationMinutes && (
                      <span className="text-[11px] text-slate-500 font-medium flex items-center gap-1">
                        ...
                        {activeTask.durationMinutes} phút
                      </span>
                    )}
                  </div>
              ...
              {activeTask.type === 'video' && (
                <div className="space-y-4">
                  ...
                  <div className="p-3 bg-blue-50/60 rounded-xl border border-blue-100 text-xs text-blue-900 flex items-center gap-2">
                    ...
                    <span>Hệ thống áp dụng cơ chế <strong>Anti-Seek (chống tua nhanh)</strong>. Hãy xem tối thiểu 90% thời lượng để mở khóa nhiệm vụ kế tiếp.</span>
                  </div>
                </div>
              )}
        ...
        {/* Right Col (4 cols): Single Pathway Playlist */}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-[10px] text-slate-500 mt-0.5 capitalize">
                          {t.type === 'video' ? '🎬 Video bài giảng' :
                           t.type === 'document' ? '📄 Tài liệu đọc' :
                           t.type === 'quiz' ? '❓ Trắc nghiệm' :
                           t.type === 'assignment' ? '💻 Bài tập nộp' : '🏆 Nghiệm thu'}
                        </div>
                      </div>
                    </div>

                    <ChevronRight className={`w-4 h-4 text-slate-400 ${isActive ? 'text-blue-600 font-bold' : ''}`} />
                  </button>
                );
              })}
            </div>
